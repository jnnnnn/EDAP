...
namespace EDAP
{
    class OpenCVExperiments
    {
        ...
        public static void Kalman()
        {
            ...
            k.ControlMatrix.SetArray(row: 0, col: 0, data: new float[,] { { 0 }, { 0 }, { 0.001f } });

            // No idea what these are, messed around with values until they seemed good
            k.MeasurementMatrix.SetIdentity();
            k.ProcessNoiseCov.SetIdentity(0.00001); // increase this for faster but noiser response
            k.MeasurementNoiseCov.SetIdentity(10); // increase this for slower and smoother response
            ...
            for (int i = 50; i < points.GetLength(0); i++)
            {
                ...
                int filteredVelocity = (int)(10 * (prevPrediction - prediction));
                ...
                graph.Line(i, prevFilteredVelocity + 250, i + 1, filteredVelocity + 250, new Scalar(255, 0, 255));
